...
# Code mostly from https://github.com/jeffreyhuang1/two-stream-action-recognition

def numpy_to_list(x):
    if len(x.shape) == 1:
       return list(x.astype(float))
...
def record_info(info,filename,mode):
    ...
    if mode =='train':
        ...
        column_names = ['Epoch','Batch Time','Data Time','Loss','Prec@1','lr', 'AUC']
    ...
    if mode =='test':
        result = (
              ...
              'Loss {loss} '
              'Prec@1 {top1} \n'.format(batch_time=info['Batch Time'],
                                        loss=info['Loss'], top1=info['Prec@1'], AUC=info['AUC']))
        print(result)
        df = pd.DataFrame.from_dict(info)
        column_names = ['Epoch','Batch Time','Loss','Prec@1','AUC']
